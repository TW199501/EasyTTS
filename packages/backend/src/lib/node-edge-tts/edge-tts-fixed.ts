...
interface TtsOptions {
  outputType?: 'stream' | 'buffer' | 'file' // 返回類型：流、Buffer 或寫入檔案
  audioPath?: string // 如果是文件輸出，則需要路徑
    ...
    _wsConnect.on('message', (data: Buffer, isBinary: boolean) => {
      ...
      if (isBinary) {
        ...
        if (outputType === 'file') {
          ...
          readableStream!.push(audioData) // 即時推送
        ...
        if (message.includes('Path:turn.end')) {
          ...
          if (outputType === 'file') {
            ...
            _wsConnect.close()
            resolveBuffer?.(audioBuffer) // 直接調用 resolve
          }
