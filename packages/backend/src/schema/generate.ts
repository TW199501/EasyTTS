import {
  ...
  DIRECT_GEN_LIMIT,
  LIMIT_TEXT_LENGTH,
  LIMIT_TEXT_LENGTH_ERROR_MESSAGE,
} from './../config/index'
...
export const edgeSchema = z.object({
  text: z.string().trim().min(5, { message: '文本最少 5 字元！' }),
...
export const llmSchema = z.object({
  ...
  openaiBaseUrl: z.preprocess(
    (val) => val ?? '',
    z.string().trim().url({ message: '請輸入有效的 OpenAI API URL！' })
  ),
  openaiKey: z.preprocess(
    (val) => val ?? '',
    z.string().trim().min(1, { message: '請在環境變數設置或前端傳入 openaiKey！' })
  ),
  openaiModel: z.preprocess(
    (val) => val ?? '',
    z.string().trim().min(1, { message: '請在環境變數設置或前端傳入 openaiModel 模型名稱！' })
  ),
  useLLM: z.boolean().default(true),
})

const dataItemSchema = z.object({
  text: z.string().min(1, '文本內容不能為空'),
  voice: z.string().min(1, '語音類型不能為空'),
...
const jsonSchema = z.object({
  data: z.array(dataItemSchema).min(1, '數據數組不能為空'),
})
...
const commonValidate = (req: Request, res: Response, next: NextFunction, schema: z.ZodTypeAny) => {
  try {
    ...
    if (LIMIT_TEXT_LENGTH) {
      ...
      if (allTxt?.length > LIMIT_TEXT_LENGTH) {
        res.status(400).json({
          code: 400,
          message:
            LIMIT_TEXT_LENGTH_ERROR_MESSAGE || `文本內容不能超出 ${LIMIT_TEXT_LENGTH} 字元哦！`,
          success: false,
        })
        return
      }
    ...
    if (error instanceof z.ZodError) {
      res.status(400).json({ code: 400, errors: error.issues, success: false })
      return
    }
...
export const validateEdge = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (isGenerate && body.text?.length > DIRECT_GEN_LIMIT) {
    res.status(400).json({
      ...
      errors: [
        {
          message: `文本長度不能超過 ${DIRECT_GEN_LIMIT} 個字元，長文本請用流式介面`,
          path: ['text'],
        },
      ],
      success: false,
...
export const validateJson = (req: Request, res: Response, next: NextFunction) => {
  const sendBufferJson = (status: number, payload: any) => {
    const buf = Buffer.from(JSON.stringify(payload))
    res.status(status).setHeader('Content-Type', 'application/json').send(buf)
  }
  try {
    ...
    if (error instanceof z.ZodError) {
      sendBufferJson(400, { code: 400, errors: error.issues, success: false })
      return
    }
    sendBufferJson(500, { code: 500, message: 'Internal server error' })
    return
  }
}
