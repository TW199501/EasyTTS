        ...
        <el-card class="settings-card">
          ...
          <div class="voice-mode-selector">
            <el-radio-group v-model="audioConfig.voiceMode" size="large">
              <el-radio-button label="preset">預設語音</el-radio-button>
              <el-tooltip
                content="通過AI推薦不同的角色語音！(實驗性功能，結果取決於模型能力！)"
                placement="top"
                effect="light"
              >
          ...
          <div v-if="audioConfig.voiceMode === 'preset'" class="voice-selector">
            <el-form label-position="top" size="default">
              ...
              <el-form-item label="語音">
                <el-select v-model="audioConfig.selectedVoice" placeholder="選擇語音" filterable>
                  <el-option
...
const languages = ref([
  ...
  { code: 'en-GB', name: '英語（英國）' },
  { code: 'en-AU', name: '英語（澳洲）' },
  { code: 'en-CA', name: '英語（加拿大）' },
])
const customColors = [
  { color: '#f5222d', percentage: 10 }, // 紅色 (開始/較低)
  { color: '#fa541c', percentage: 20 }, // 橘紅
  { color: '#fa8c16', percentage: 30 }, // 橘黃
  { color: '#fadb14', percentage: 40 }, // 黃色
  { color: '#fadb14', percentage: 50 }, // 黃色 (中間狀態)
  { color: '#a0d911', percentage: 60 }, // 酸橙綠
  { color: '#73d13d', percentage: 70 }, // 淺綠
  { color: '#52c41a', percentage: 80 }, // 綠色
  { color: '#52c41a', percentage: 90 }, // 綠色 (接近完成)
  { color: '#52c41a', percentage: 100 }, // 純綠 (完成)
]

const handleClose = (realClose: () => void) => {
  if (generating.value) {
    ElMessageBox.confirm('確定關閉嗎，這將停止當前的生成任務', '操作提示', {
...
const reset = () => {
  ElMessageBox.confirm('確定將配置重設為初始狀態', '操作提示', {
    confirmButtonText: '確定',
    cancelButtonText: '取消',
    type: 'warning',
  }).then(() => {
...
const handle400 = (error: AxiosError) => {
  ...
  if (message) {
    if (message === 'English model cannot process non-English text') {
      ElMessage.error(`英文模型不支持轉中文語音哦！請切換模型到中文！`)
    } else {
...
const handle429 = (error: unknown) => {
  if (error instanceof AxiosError) {
    if (error.status === 429) {
      ElMessage.error('請求太快啦，小伺服器扛不住！請稍後再試')
    }
  }
}
...
const handleFile = (file: any) => {
  ...
  reader.onerror = () => {
    ElMessage.error('文件讀取錯誤，請上傳 txt 文本！')
  }
...
const beforeUnloadHandler = async (event: BeforeUnloadEvent) => {
  ...
  if (generationStore.audioList.length > 0) {
    ...
    event.returnValue = '操作將刪除頁面上的所有音訊檔案，請確認已經下載！'
    return event.returnValue
  }

  if (generationStore.audioList.length > 0) {
    try {
      await ElMessageBox.confirm('操作將刪除頁面上的所有音訊檔案，請確認已經下載！', '操作提示', {
        confirmButtonText: '確定',
        cancelButtonText: '取消',
        type: 'warning',
      })
