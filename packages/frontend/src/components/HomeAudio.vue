...
const colorMap = {
  徐鳳年: (opacity = 1) => `rgba(255, 135, 135, ${opacity})`,
  ...
  盧白擷: (opacity = 1) => `rgba(142, 158, 239, ${opacity})`,
}
...
const voiceSegments = [
  {
    ...
    avatar: zhMale,
    color: getVoiceColor('徐鳳年', 0.3),
    textColor: getVoiceColor('徐鳳年'),
  },
  {
    voice: '姜泥',
    start: 4.337,
  ...
  },
  {
    ...
    color: getVoiceColor('盧白擷', 0.3),
    textColor: getVoiceColor('盧白擷'),
  },
]
...
const seek = (event: MouseEvent) => {
  ...
  const updatePosition = (e: MouseEvent) => {
    ...
    progressPercent.value = percent * 100 // 即時更新進度百分比
